...
let urlify = (string) =>
  string.toLowerCase().split(/\s+/).join('-');
...
let functionalFilter = (states) =>
  states.filter(state => state.split(/\s+/).length === 2);
let dakotas = (states) =>
  states.filter(state => state.includes("Dakota"));
...
// exercise reduce: Functional solution for products
